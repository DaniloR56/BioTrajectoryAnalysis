...
The BioTrajectoryAnalysis project currently consists of two main Python
programs:

\begin{itemize}
\item \texttt{AntTracking.py}: extraction of trajectories from video
recordings using computer vision techniques.

\item \texttt{AntTrajectoryAnalysis.py}: quantitative analysis of
trajectory datasets using methods from statistical physics and movement
ecology.
\end{itemize}

The analysis module includes occupancy maps, Monte Carlo area
estimation, Principal Component Analysis (PCA), Mean Squared
Displacement (MSD), straightness indices, fractal dimension estimation,
and statistical uncertainty analysis.

The results reported in the accompanying publication were generated
using AntTrajectoryAnalysis version 1.1.
...
The program \texttt{AntTracking.py}  extracts trajectories from video recordings.
...
\begin{verbatim}
python AntTracking.py video.mov
    --threshold 18
    --min-area 6
    --max-area 250
